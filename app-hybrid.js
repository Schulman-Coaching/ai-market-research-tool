class AIMarketResearchTool {
    constructor() {
        ...
        this.selectedPrototypeService = 'openrouter'; // Default to OpenRouter
    ...
    initializeEventListeners() {
        ...
        // Prototype service change handler
        document.addEventListener('change', (e) => {
            if (e.target.id === 'prototype-service') {
    ...
    checkApiKeys() {
        ...
        const storedService = localStorage.getItem('prototype-service');
        const storedModel = localStorage.getItem('selected-model');
        
        if (storedGeminiKey) this.geminiApiKey = storedGeminiKey;
        if (storedPerplexityKey) this.perplexityApiKey = storedPerplexityKey;
        if (storedOpenrouterKey) this.openrouterApiKey = storedOpenrouterKey;
        if (storedService) this.selectedPrototypeService = storedService;
    ...
    showApiModal() {
        ...
        document.getElementById('prototype-service').value = this.selectedPrototypeService;
        
        this.updateModelOptions(this.selectedPrototypeService);
        document.getElementById('selected-model').value = this.selectedModel;
    }
    ...
    updateModelOptions(service) {
        ...
        const models = {
            'openrouter': [
                { value: 'anthropic/claude-3.5-sonnet', name: 'Claude 3.5 Sonnet (Recommended)' },
                { value: 'openai/gpt-4o', name: 'GPT-4o' },
                { value: 'openai/gpt-4o-mini', name: 'GPT-4o Mini' },
                { value: 'meta-llama/llama-3.1-405b-instruct', name: 'Llama 3.1 405B' },
                { value: 'google/gemini-pro-1.5', name: 'Gemini Pro 1.5' }
            ],
            'perplexity': [
                { value: 'sonar-small-chat', name: 'Sonar Small Chat' },
                { value: 'sonar-medium-chat', name: 'Sonar Medium Chat' },
                { value: 'sonar-large-chat', name: 'Sonar Large Chat' }
            ],
            'gemini': [
                { value: 'gemini-1.5-flash-latest', name: 'Gemini 1.5 Flash' },
                { value: 'gemini-1.5-pro-latest', name: 'Gemini 1.5 Pro' }
            ]
        };

        models[service].forEach(model => {
    ...
    saveApiKeys() {
        ...
        const prototypeService = document.getElementById('prototype-service').value;
        const selectedModel = document.getElementById('selected-model').value;

        if (!geminiKey) {
            alert('Gemini API key is required for market research');
            return;
        }

        // Check if at least one prototype service key is provided
        if (prototypeService === 'perplexity' && !perplexityKey) {
            alert('Perplexity API key is required for the selected service');
            return;
        }

        if (prototypeService === 'openrouter' && !openrouterKey) {
            alert('OpenRouter API key is required for the selected service');
            return;
        }
        ...
        this.selectedPrototypeService = prototypeService;
        ...
        localStorage.setItem('prototype-service', prototypeService);
    ...
    async startResearchProcess() {
        ...
        // Check prototype service requirements
        if (this.selectedPrototypeService === 'perplexity' && !this.perplexityApiKey) {
            this.showApiModal();
            return;
        }

        if (this.selectedPrototypeService === 'openrouter' && !this.openrouterApiKey) {
            this.showApiModal();
            return;
        }
...
Format your response as a JSON array with objects containing 'title' and 'description' fields.`;
    ...
    async generatePrototypes() {
        ...
        this.updateStatus(`Generating prototypes with ${this.selectedPrototypeService.toUpperCase()}...`);
        ...
        for (let i = 0; i < this.extractedIdeas.length; i++) {
            ...
            try {
                ...
                switch (this.selectedPrototypeService) {
                    case 'openrouter':
                        response = await this.callOpenRouterAPI(prototypePrompt);
                        break;
                    case 'perplexity':
                        response = await this.callPerplexityAPI(prototypePrompt);
                        break;
                    case 'gemini':
                        response = await this.callGeminiAPI(prototypePrompt);
                        break;
                    default:
                        throw new Error('Invalid prototype service selected');
                }
                
                this.prototypeResults.push({
                    idea: idea,
                    prototype: response,
                    service: this.selectedPrototypeService,
...
Focus on creating a practical, implementable solution that could be developed as a modern web application.`;
        ...
        try {
            const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                ...
                headers: {
                    ...
                    'X-Title': 'AI Market Research Tool'
                },
                body: JSON.stringify({
                    model: this.selectedModel,
                    messages: [
    ...
    async callGeminiAPI(prompt) {
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${this.selectedModel}:generateContent?key=${this.geminiApiKey}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    completeWorkflow() {
        ...
        document.getElementById('current-status').innerHTML = `
            <div class="text-green-600 text-center">
                ...
                <p class="text-sm text-gray-600 mt-2">Generated ${this.extractedIdeas.length} product ideas and ${this.prototypeResults.length} prototypes using ${this.selectedPrototypeService.toUpperCase()}</p>
            </div>
        `;
            ...
            prototypeCard.innerHTML = `
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <span class="text-xs bg-green-200 text-green-800 px-2 py-1 rounded">${result.service.toUpperCase()}</span>
    ...
    exportPrototypes() {
        ...
        this.prototypeResults.forEach((result, index) => {
            ...
            content += `**Service:** ${result.service.toUpperCase()}\n`;
